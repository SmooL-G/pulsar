    ...
    <div className="flex flex-col h-full bg-white dark:bg-dark-700">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-6">
        ...
        <div className="space-y-1">
          {isGroup && (
            <InfoAction icon={<Users size={18} />} label={t('info.members')} value={`${(activeChat as any).memberCount || 0}`} />
          )}
          <InfoAction icon={<Bell size={18} />} label={t('info.notifications')} value={t('info.on')} />
          {isGroup && activeChat.inviteCode && (
            <InfoAction icon={<LinkIcon size={18} />} label={t('info.inviteLink')} value={t('info.copy')} />
          )}
