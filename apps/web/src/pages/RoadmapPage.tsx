...
const PHASES: Phase[] = [
  ...
  },
  {
    ...
    features: [
      ...
      { text: { en: 'Voice messages with live waveform', ru: 'Голосовые сообщения с live-волной микрофона' }, status: 'done', icon: <Mic size={14} /> },
      { text: { en: 'Drag & drop files + paste image from clipboard', ru: 'Drag & drop файлов + вставка изображения из буфера' }, status: 'done' },
  ...
  },
  {
    ...
    features: [
      ...
      { text: { en: 'PLS transaction history (deposits, transfers, burns, rewards)', ru: 'История операций PLS (депозиты, переводы, сжигание, награды)' }, status: 'done' },
  ...
  },
  {
    ...
    features: [
      ...
      { text: { en: 'Premium chat wallpapers (5 gradients)', ru: 'Premium обои чата (5 градиентов)' }, status: 'done' },
      { text: { en: 'Premium voice transcription (OpenAI Whisper, multilingual)', ru: 'Premium расшифровка голосовых (OpenAI Whisper, любой язык)' }, status: 'done', icon: <Sparkles size={14} /> },
      { text: { en: 'RUB top-up via YooKassa (5 packages, up to 40% volume discount)', ru: 'Пополнение PLS картой через ЮKassa (5 пакетов, скидка до 40%)' }, status: 'done', icon: <Gem size={14} /> },
  ...
  },
  {
    ...
    features: [
      ...
      { text: { en: 'Cloudflare DNS + Email Routing (mail on the domain)', ru: 'Cloudflare DNS + Email Routing (почта на домене)' }, status: 'done' },
      { text: { en: 'Resend SMTP (password reset, verification, notifications)', ru: 'Resend SMTP (сброс пароля, верификация, нотификации)' }, status: 'done' },
      { text: { en: 'Case-insensitive email login & registration', ru: 'Вход и регистрация без учёта регистра email' }, status: 'done' },
  ...
  {
    title: { en: 'UX Polish & Personalization', ru: 'Полировка UX и персонализация' },
    subtitle: { en: 'Daily-use refinements', ru: 'Ежедневные доработки' },
    status: 'done', date: 'June 2026', highlight: true,
    features: [
      { text: { en: '5-tab BottomNav shell with notification badges (chat/contacts/wallet/home)', ru: '5-табовая нижняя навигация с бейджами (чат/контакты/кошелёк/главная)' }, status: 'done' },
      { text: { en: 'Centered chat column on wide screens, hidden scrollbar', ru: 'Центрированная колонка чата на широких экранах, скрытый скроллбар' }, status: 'done' },
      { text: { en: 'Glassmorphic rounded chat header', ru: 'Скруглённый glass-хедер чата' }, status: 'done' },
      { text: { en: 'Glassmorphic pinned-message banner', ru: 'Glass-баннер закреплённого сообщения' }, status: 'done' },
      { text: { en: 'Floating "jump to bottom" pill', ru: 'Плавающая пилюля «вниз к последнему»' }, status: 'done' },
      { text: { en: 'Hold-to-record voice messages (Telegram-style + tap-to-lock)', ru: 'Запись голосовых на удержание (как в TG + защёлка)' }, status: 'done', icon: <Mic size={14} /> },
      { text: { en: 'Pin/unpin messages with FIFO limit & socket sync', ru: 'Закреп/открепление сообщений с FIFO-лимитом и sync через сокет' }, status: 'done' },
      { text: { en: '"Unread messages" divider in chat (Telegram-style)', ru: 'Разделитель «Новые сообщения» в чате (как в TG)' }, status: 'done' },
      { text: { en: 'Delete-for-everyone (24h window + admin override)', ru: 'Удаление для всех (окно 24ч + админ-override)' }, status: 'done' },
      { text: { en: 'Runtime primary-color theme picker (5 palettes via CSS vars)', ru: 'Смена основной темы на лету (5 палитр через CSS-переменные)' }, status: 'done' },
      { text: { en: 'Global keyboard-shortcut framework', ru: 'Глобальный фреймворк горячих клавиш' }, status: 'done' },
      { text: { en: 'Per-user HomeTab tile visibility (show/hide cards)', ru: 'Скрытие плиток главной по выбору пользователя' }, status: 'done' },
      { text: { en: 'Profile page: avatar upload + social links', ru: 'Профиль: загрузка аватара + соц-ссылки' }, status: 'done' },
  ...
  },
  {
    ...
    subtitle: { en: 'Decentralized signaling + relay layer', ru: 'Децентрализованный signaling и relay-слой' },
    status: 'active', date: 'Q2 2026',
    features: [
      { text: { en: 'Tauri desktop node app (Win/Linux)', ru: 'Десктоп-нода на Tauri (Win/Linux)' }, status: 'done' },
      { text: { en: 'PLS rewards for uptime + bandwidth + peers', ru: 'PLS-награды за uptime + bandwidth + пиры' }, status: 'done' },
      ...
      { text: { en: 'Message + media cache layer on nodes', ru: 'Кэш сообщений и медиа на нодах' }, status: 'planned' },
    ],
  },
];
